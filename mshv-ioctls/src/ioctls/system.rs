...
impl Mshv {
    ...
    fn get_host_partition_property_ex(&self, property_code: u32) -> Result<Box<hv_partition_property_ex>> {
    ...
    pub fn get_vmm_capabilities(&self) -> Result<hv_partition_property_vmm_capabilities> {
        match self.get_host_partition_property_ex(
            ...
            Ok(output) => {
                ...
                let vmm_caps = unsafe { output.vmm_capabilities.clone() };
                Ok(vmm_caps)
            }
            Err(MshvError::Hypercall { status, .. }) if status == Some(HvError::AccessDenied) => {
...
mod tests {
    ...
    #[test]
    #[ignore]
    fn test_get_vmm_caps() {
        let hv = Mshv::new().unwrap();
        let vmm_caps = hv.get_vmm_capabilities();
        assert!(vmm_caps.is_ok());

        let vmm_caps = vmm_caps.unwrap();
        // SAFETY: accessing union fields
        let caps: u64 = unsafe {
           vmm_caps.__bindgen_anon_1.as_uint64[0]
        };
        println!("VMM capabilities retrieved successfully: 0x{:x}", caps);
    }
}
